...
from pybaseball import statcast_batter_daily  # <- Correct import
import time
...
with tab1:
    ...
    if matchups:
        ...
        if st.button("⚡ Calculate + Rank", key="season_calculate"):
            with st.spinner("Running season model..."):
                try:
                    from scrape_stats import run_scrape  # Lazy import to avoid issues
...
# --- 7-DAY TAB ---
with tab2:
    st.header("7-Day Stat Model")
    if matchups:
        selected_matchup_7d = st.selectbox("Select Today's Matchup (7-Day)", matchups, key="7d_matchup")
        team1_7d, team2_7d = selected_matchup_7d.split(" @ ")

        st.markdown("### 🎯 7-Day Stat Weights")
        available_stats_7d = ["EV", "Barrel %", "xSLG"]
        ...
        for i in range(3):
            cols = st.columns([2, 1])
            stat = cols[0].selectbox(f"Stat {i+1} (7D)", available_stats_7d, index=i, key=f"stat7d_{i}")
            weight = cols[1].number_input(f"Weight {i+1} (7D)", min_value=0.0, max_value=1.0, value=default_weights[i], step=0.01, key=f"w7d_{i}")
            stat_selections_7d.append(stat)
            weight_inputs_7d.append(weight)

        if st.button("⚡ Calculate + Rank (7-Day)", key="7d_calculate"):
                ...
                try:
                    ...
                    for name in batters:
                        try:
                            ...
                            row = id_map[(id_map['name_first'] + " " + id_map['name_last']).str.lower() == name.lower()]
                            if not row.empty:
                                player_id = int(row['key_mlbam'].values[0])
                                end_date = datetime.now()
                                start_date = end_date - pd.Timedelta(days=7)
                                data = statcast_batter_daily(start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), player_id)
                                if data.empty:
                                    continue
                                avg_ev = data['launch_speed'].mean()
                                barrel_pct = (data['launch_speed'] > 95).mean()
                                xslg = data['estimated_slg'].mean()
                                stats = {
                                    "EV": avg_ev,
                                    "Barrel %": barrel_pct * 100,
                                    "xSLG": xslg
                                }
                                score = sum(stats[stat] * weight_inputs_7d[i] for i, stat in enumerate(stat_selections_7d) if stat in stats)
